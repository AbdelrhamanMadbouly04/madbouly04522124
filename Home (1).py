...
def simulate_torrefaction(biomass, moisture, temp_C, duration_min, size, initial_mass_kg):
    ...
    # حماية من القيم السالبة في المدخلات
    ...
    def model(y, t):
        m, v = y # m: نسبة الرطوبة المتبقية، v: نسبة المواد المتطايرة المتبقية
        
        # معادلة التجفيف: تتوقف تماماً إذا وصلت الرطوبة للصفر
        if m <= 0.0001: # إذا كانت الرطوبة شبه منعدمة
            d_m = 0.0
            m = 0.0 # تثبيت القيمة عند الصفر لمنع القيم السالبة
        else:
            d_m = -k_drying * m
            
        # معادلة التحلل الحراري للمواد المتطايرة
        if v <= 0.0001:
            d_v = 0.0
        else:
            d_v = -k_devol_eff * v
    ...
    # تنظيف النتائج (فرض الصفر على أي قيم سالبة صغيرة جداً ناتجة عن الحساب الرقمي)
    sol[:, 0] = np.maximum(sol[:, 0], 0.0) # Moisture Profile
    sol[:, 1] = np.maximum(sol[:, 1], 0.0) # Volatiles Profile

    # --- حساب المتبقيات النهائية والكتل ---
    
    final_moisture_fraction = sol[-1, 0]
    final_volatiles_fraction = sol[-1, 1]

    # حساب الكسور المفقودة (التي تحولت لغاز أو بخار)
    # هذه القيم لن تتجاوز الكمية الابتدائية أبداً بفضل الـ ODE
    ...
    # حساب كتلة الفحم الحيوي الصلب (Biochar Yield)
    # الكتلة الصلبة النهائية = (الرماد + المواد المتطايرة المتبقية + أي رطوبة لم تتبخر)
    # أو بطريقة أخرى: 1.0 - المفقودات
    final_solid_fraction = 1.0 - moisture_lost_fraction - volatiles_lost_fraction
    
    # تحويل الكسور إلى كتل (kg)
    yields_data = {
        ...
        "Ash (Inside Biochar)": ash_content * initial_mass_kg # للعلم فقط
    }
    ...
    # حساب تكوين الغازات (افتراضي بناء على البيانات)
...
def main():
    ...
    # منطق التحقق من اكتمال التجفيف
    initial_water_mass = initial_mass * moisture_content / 100
    if water_mass < initial_water_mass * 0.99:
        st.warning(f"⚠️ Note: Drying is incomplete. Consider increasing time or temperature. (Evaporated: {water_mass:.1f} kg / Total Moisture: {initial_water_mass:.1f} kg)")
    else:
    ...
    with tab2:
        ...
        st.caption("The graph shows the decay of Moisture and Volatiles over time. Notice moisture hits zero and stays there.")
...
# --- 4. PDF Generator (Simplified without complex CSS) ---
